...
describe Spoonerise::Spoonerism do
  context 'when initialized' do
    ...
    context 'with not enough flip-able words' do
      ...
      it { expect { Spoonerise::Spoonerism.new(
    ...
    context 'with the words "the ultimate spoonerise test"' do
      ...
      context 'and excluding too many words' do
        let(:too_many_excl) do
          Spoonerise::Spoonerism.new(words, {reverse: true, exclude: %w(the ultimate test)})
        end
      ...
      context 'and no options' do
        ...
        context 'and the instance is saved' do
          let(:saved) { Spoonerise::Spoonerism.new(words, {save: true}) }
          it { expect { saved.save }.to output(/INFO -- :/).to_stdout }
        end
      end
      context 'and reversed' do
        let(:reversed) { Spoonerise::Spoonerism.new(words, {reverse: true}) }
      ...
      context 'and excluding "test"' do
        let(:excluding) { Spoonerise::Spoonerism.new(words, {exclude: %w(test)}) }
      ...
      context 'and reversed, excluding "test"' do
        let(:rev_excl) { Spoonerise::Spoonerism.new(words, {reverse: true, exclude: %w(test)}) }
      ...
      context 'and lazy' do
        let(:lazy) { Spoonerise::Spoonerism.new(words, {lazy: true}) }
      ...
      context 'and lazy, excluding "test"' do
        let(:lazy_excl) { Spoonerise::Spoonerism.new(words, {lazy: true, exclude: %w(test)}) }
      ...
      context 'and lazy, reversed' do
        let(:lazy_rev) { Spoonerise::Spoonerism.new(words, {lazy: true, reverse: true}) }
      ...
      context 'and lazy, reversed, excluding "test"' do
        let(:lazy_excl_rev) { Spoonerise::Spoonerism.new(words, {lazy: true, reverse: true, exclude: %w(test)}) }
